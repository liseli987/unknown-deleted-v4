...
client.on("message" , async msg => {
 ...
 if(afk){
   ...
   if(msg.content.includes(kisi3)){
       ...
       msg.reply (`Etiketlediğiniz Kişi Afk Sebep : ${sebep}`)
   }
 }
  if(msg.author.id === kisi){

       msg.reply(`Afk'lıktan çıktınız. <a:yesiltik:794763689809936405>`)
...
let rol = member.guild.roles.cache.find(r => r.name === "797151448264736798");
...
client.on('guildMemberAdd', async(member) => {
let mute = member.guild.roles.cache.find(r => r.name === "730414874995458148");
...
client.on("message", message => {
    if(message.content.toLowerCase() == "v!tag") 
    return message.channel.send(`TAG`)
});
